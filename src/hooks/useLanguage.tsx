...
export const useLanguage = (langArray: [], flavorArray: [] | undefined) => {
  ...
  bilingual = langArray?.filter((countryLang: IPokemonLanguage) => countryLang.language.name === language);
  let flavorText: IPokemonFlavorText[] | undefined = [];
  flavorText = flavorArray?.filter((featureText: IPokemonLanguage) => featureText.language.name === language);
